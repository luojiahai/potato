...
Screen layouts are pinned by the golden frames in `testdata/frames`, and the generated shell glue by `testdata/init`. Regenerate the frames with `go test ./internal/tui -update-frames` after a deliberate visual change, then read the diff. The two screenshots are the real binary, not a drawing of it: `scripts/screenshots.sh` seeds a fixture library into a throwaway `POTATO_INSTALL`, builds potato into it, and gives [vhs](https://github.com/charmbracelet/vhs) (`brew install vhs`) a terminal to run it in and photograph. Retaking them is reproducible, so a diff shows what the screens changed — plus the version on the header rule, which follows the latest tag. To try a real install without touching your own `~/.potato`, point `POTATO_INSTALL` at a throwaway directory; `internal/shell` documents the rest.
